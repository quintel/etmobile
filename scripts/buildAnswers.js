...
const writeResults = (path, results) => {
  // Build the JSON output manually rather than with JSON.stringify so as to
  // order the output by key.
  const data = Object.keys(results).sort().map(
    key => `  "${key}": ${results[key]}`
  ).join(',\n');
...
const serialFetchResults = (baseline, results, requests, index, url) => {
  const { inputs, key } = requests[index];
...
const choicesToRequests = (choices) => {
  ...
  choices.forEach(({ key, inputs }, index) => {
    requests.push({ index, inputs, key });
  });
